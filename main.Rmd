...
credit_data <- list(N_train=nrow(X_train_pooled), 
...
pooled_fit <- stan(file="pooled.stan", data=credit_data, chains=4, iter=2000, warmup=1000)
```
...
```{r}
pooled_draws <- as_draws_df(pooled_fit)
hist(pooled_draws$`beta_0`)
hist(pooled_draws$`beta[2]`)

hierarchical_draws <- as_draws_df(hierarchical_fit)
hist(hierarchical_draws$`beta_0[1]`)
hist(hierarchical_draws$`beta[2,1]`)
```
